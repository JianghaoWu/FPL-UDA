...
class ModelLossSemsegGatedCRF3D(torch.nn.Module):
    ...
    def forward(
            self, y_hat_softmax, kernels_desc, kernels_radius, sample, depth_input, height_input, width_input, spacing, 
            mask_src=None, mask_dst=None, compatibility=None, custom_modality_downsamplers=None, out_kernels_vis=False
    ):
        """
        kernels_radius is a list !!!!!!!!!!!
        spacing is like [torch.tensor([1.5, 1.5]), torch.tensor([0.5, 0.4]), torch.tensor([0.5, 0.4])]
        """
        assert len(kernels_radius) == 3 and len(spacing) == 3
        assert y_hat_softmax.dim() == 5, 'Prediction must be a NCDHW batch'
        N, C, depth_pred, height_pred, width_pred = y_hat_softmax.shape
        assert spacing[0].shape[0] == N
        device = y_hat_softmax.device

        assert depth_input % depth_pred == 0 and width_input % width_pred == 0 and height_input % height_pred == 0 \
            and width_input * height_pred == height_input * width_pred, \
            f'[{width_input}x{height_input}] !~= [{width_pred}x{height_pred}]'
        kernels = self._create_kernels(
            kernels_desc, kernels_radius, sample, N, depth_pred, height_pred, width_pred, spacing, device, custom_modality_downsamplers
        )
        ...
        product_kernel_x_y_hat = product_kernel_x_y_hat.reshape(N, C, depth_pred, height_pred, width_pred)
    ...
    @staticmethod
    def _downsample(img, modality, depth_dst, height_dst, width_dst, custom_modality_downsamplers):
        ...
        return f_down(img, (depth_dst, height_dst, width_dst))
            ...
            for modality, sigma in desc.items():
                ...
                if modality == 'xy':
                    feature = ModelLossSemsegGatedCRF3D._get_mesh(
                        N, depth_pred, height_pred, width_pred, spacing, device)
                else:
                    ...
                    feature = ModelLossSemsegGatedCRF3D._downsample(
                        feature, modality, depth_pred, height_pred, width_pred, custom_modality_downsamplers
                    )
                feature /= sigma
    ...
    def _create_kernels_from_features(features, radius):
        assert features.dim() == 5, 'Features must be a NCDHW batch'
    ...
    @staticmethod
    def _get_mesh(N, D, H, W, spacing, device):
        mesh_D = torch.arange(0, D, 1, dtype=torch.float32, device=device).reshape(1, 1, D, 1, 1).repeat(N, 1, 1, H, W)
        mesh_H = torch.arange(0, H, 1, dtype=torch.float32, device=device).reshape(1, 1, 1, H, 1).repeat(N, 1, D, 1, W)
        mesh_W = torch.arange(0, W, 1, dtype=torch.float32, device=device).reshape(1, 1, 1, 1, W).repeat(N, 1, D, H, 1)
        mesh_D = mesh_D * (spacing[0].reshape(N, 1, 1, 1, 1).repeat(1, 1, D, H, W))
        mesh_H = mesh_H * (spacing[1].reshape(N, 1, 1, 1, 1).repeat(1, 1, D, H, W))
        mesh_W = mesh_W * (spacing[2].reshape(N, 1, 1, 1, 1).repeat(1, 1, D, H, W))
        return torch.cat((mesh_D, mesh_H, mesh_W), 1)

    @staticmethod
    def _unfold(img, radius):
        assert img.dim() == 5, 'Unfolding requires NCDHW batch'
        N, C, D, H, W = img.shape
...
class ModelLossSemsegGatedCRF3D22D(torch.nn.Module):
    ...
    def forward(
            self, y_hat_softmax, kernels_desc, kernels_radius, sample, depth_input, height_input, width_input,
            mask_src=None, mask_dst=None, compatibility=None, custom_modality_downsamplers=None, out_kernels_vis=False
    ):
        """
        kernels_radius is a list !!!!!!!!!!!
        ...
        y_hat_softmax = y_hat_softmax.permute(0, 2, 1, 3, 4).reshape(N * depth_pred, C, height_pred, width_pred)
        sample = sample.permute(0, 2, 1, 3, 4).reshape(N * depth_input, C_input, height_input, width_input)
        kernels = self._create_kernels(
        ...
        kernels_size = [x * 2 + 1 for x in kernels_radius]
        product_kernel_x_y_hat = (kernels * y_hat_unfolded) \
            .reshape(N * depth_pred, C, kernels_size[1] * kernels_size[2], -1).sum(dim=2, keepdim=False)
    ...
    def _create_kernels_from_features(features, radius):
        ...
        kernels = kernels - kernels[:, :, radius[1],
                                    radius[2], :].reshape(N, C, 1, 1, -1)
        kernels = (-0.5 * kernels ** 2).sum(dim=1, keepdim=True).exp()
        kernels[:, :, radius[1], radius[2], :] = 0
        return kernels
    ...
    def _unfold(img, radius):
        N, C, H, W = img.shape
        diameter = [x * 2 + 1 for x in radius]
        return unfoldNd.unfoldNd(img, kernel_size=(diameter[1], diameter[2]), dilation=1, padding=(radius[1], radius[2]), \
            stride=1).reshape(N, C, diameter[1], diameter[2], -1)